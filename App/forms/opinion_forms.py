#id_opinion, username_opción, id_product, rating_product, comment_opinion, date_opinion
import datetime
...
class OpinionForm(FlaskForm):
    ...
    rating_product = StringField('Calificación', validators=[DataRequired()])
...
class UpdateOpinionForm(FlaskForm):
    username_opción = StringField('Nombre de Usuario', validators=[DataRequired(), Length(min=2, max=80)])
    id_product = SelectField('Producto', coerce=int ,validators=[DataRequired()], choices=[])
    rating_product = SelectField('Calificación', validators=[DataRequired()])
    comment_opinion = TextAreaField('Opinion', validators=[DataRequired(), Length(min=2, max=200)])
